...
public class BotonJuego {
	...
	public BotonJuego(TextureRegion reg, float x, float y, OrthographicCamera cam) {
		this.reg = reg;
		this.x = x;
		this.y = y;
		this.cam = cam;
		...
		Texture tex = Juego.res.getTexture("numeros");
		font = new TextureRegion[11];
		for(int i = 0; i < 6; i++) {
			font[i] = new TextureRegion(tex, 2 + i * 9, 2, 8, 8);
		}
		for(int i = 0; i < 5; i++) {
			font[i + 6] = new TextureRegion(tex, 2 + i * 9, 2, 8, 8);
		}
	...
	public void update(float dt) {
		vec.set(MiEntrada.x, MiEntrada.y, 0);
		cam.unproject(vec);
		
		if(MiEntrada.isPressed() &&
